...
SELECT 
    ...
    ROUND(t.avg_daily_sales * 7 + (t.sales_volatility * 2), 2) AS safety_stock_reorder, -- 95% confidence safety stock
...
SELECT 
    ...
    s1.Category,
    s1.avg_daily_sales,
    s1.avg_daily_revenue,
    ROUND(
        (s1.avg_daily_sales - LAG(s1.avg_daily_sales) OVER (PARTITION BY s1.Category ORDER BY s1.Seasonality)) 
        / NULLIF(LAG(s1.avg_daily_sales) OVER (PARTITION BY s1.Category ORDER BY s1.Seasonality), 0) * 100, 2
    ) AS growth_pct
FROM seasonal_avg s1
ORDER BY s1.Category, s1.Seasonality;
...
SELECT 
    ...
    ROUND(
        (
            AVG(CASE WHEN e.Holiday_Promotion = 1 THEN i.Units_Sold ELSE NULL END) - 
            AVG(CASE WHEN e.Holiday_Promotion = 0 THEN i.Units_Sold ELSE NULL END)
        ) / NULLIF(AVG(CASE WHEN e.Holiday_Promotion = 0 THEN i.Units_Sold ELSE NULL END), 0) * 100, 2
    ) AS lift_pct
FROM inventory_facts i
JOIN products p ON i.Product_ID = p.Product_ID
JOIN environment_facts e ON i.Date = e.Date AND i.Store_ID = e.Store_ID
